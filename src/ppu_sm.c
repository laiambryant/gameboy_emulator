...
void increment_ly() {    if (window_visible() && lcd_get_context()->ly >= lcd_get_context()->win_y &&
...
void load_line_sprites() {    int cur_y = lcd_get_context()->ly;
...
void ppu_mode_oam() {    if (ppu_get_context()->line_ticks >= 80) {
...
void ppu_mode_xfer() {    pipeline_process();
...
void ppu_mode_vblank() {    if (ppu_get_context()->line_ticks >= TICKS_PER_LINE) {
        increment_ly();
...
void ppu_mode_hblank() {    if (ppu_get_context()->line_ticks >= TICKS_PER_LINE) {
        increment_ly();
