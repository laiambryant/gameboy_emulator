...
void test_adds() {
	CPU* cpu = get_cpu();
	set_register(cpu->registers, A, 0x12);
	set_register_combination(cpu->registers, H, L, 0x1212);
	ADD_COMPOUND(cpu, A, H, L);
	print_registers(cpu->registers);
	assert(get_register(cpu->registers, L) == 0x24);
	assert(get_register_combination(cpu->registers, H, L) == 0x1224);
	destroy_cpu(cpu);
}